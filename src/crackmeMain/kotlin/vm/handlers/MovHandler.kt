...
class MovHandler : Handler<Mov>() {
  ...
  override fun handle(vm: VM, currentEip: Int, instruction: Mov): Int {
    GenericTwoOperandsInstructionHandler.handle(
      ...
      handle_Reg_StackC32 = { dest, src, eip -> vm.registers[dest.index] = getVmMemoryValueByConstant(vm, eip, src as Memory<Constant>) },
      handle_Reg_MemReg = { dest, src, eip -> vm.registers[dest.index] = getVmMemoryValueByRegister(vm, eip, src) },
      handle_Reg_StackReg = { dest, src, eip -> vm.registers[dest.index] = getVmMemoryValueByRegister(vm, eip, src) },
      ...
      handle_StackReg_Reg = { dest, src, eip -> putVmMemoryValueByRegister(dest, vm, vm.registers[src.index], eip) },
      ...
      handle_StackC32_Reg = { dest, src, eip -> putVmMemoryValueByConstant(dest as Memory<Constant>, vm, vm.registers[src.index], eip) },
      handle_MemReg_Const = { dest, src, eip -> putVmMemoryValueByRegister(dest, vm, getConstantValueFromVmMemory(vm, src), eip) },
      handle_StackReg_Const = { dest, src, eip -> putVmMemoryValueByRegister(dest, vm, getConstantValueFromVmMemory(vm, src), eip) },
